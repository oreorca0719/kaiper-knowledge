...
| 항목 | 결과 |
|---|---|
| 22문항 회귀 | **22/22 정상 응답**, 출처 부착 |
| 사실 정확도 (원문 대조 10문항) | **10/10** |
| 환각 검증 (코퍼스에 없는 질문 3건) | **3/3 정상 거절** |
| 평균 응답 시간 | **11.3초** |
| 프롬프트 내부 노출 | 0건 |
...
# 가입 정책 (비어 있으면 자동 가입 전면 차단 — 기본이자 가장 안전)
SIGNUP_ALLOWED_DOMAINS=     # 예: pron.co.kr,kaiper.local
```
