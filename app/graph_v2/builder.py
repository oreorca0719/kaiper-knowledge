...
def _no_retrieval_answer_node(state: GraphState) -> dict:
    ...
    【범위 통제가 여기에 있는 이유】
    라우터에서 검색 전 "범위 밖" 판정을 제거했다 (router.py 주석 참조).
    그 결과 사내 무관 질의가 no_retrieval 로 분류되어 이 노드에 도달한다.
    이 노드는 검색을 거치지 않으므로 **모델의 일반 지식이 그대로 나간다**.

    실측 누출: "점심 메뉴 추천해줘"
      -> "오늘같이 날씨가 애매한 날엔 국물 요리(김치찌개, 순두부찌개)나..."

    사내 어시스턴트가 일반 상식·코딩·추천에 답하는 것은 제품 범위를 벗어난다.
    이 지점이 유일한 누출 경로이므로 여기서 막는다. 라우터의 사전 추측과 달리
    이 판정은 "검색이 필요 없다고 이미 판정된 질의" 만 대상으로 하므로
    문서에 답이 있는 질문을 잘못 거절할 위험이 없다.
    """
    user_input = state.input_data or ""
    sys = (
        "당신은 Kaiper AI 사내 어시스턴트입니다. 사내 문서 검색·파일 분석·기능 안내만 담당합니다.\n\n"
        "【답변 가능】\n"
        "- 인사말 (안녕하세요, 반갑습니다 등)\n"
        "- 이 어시스턴트가 무엇을 할 수 있는지에 대한 질문\n"
        "위 두 가지는 짧고 정중하게 답하고, 사내 문서 검색·파일 분석을 도울 수 있다고 안내한다.\n\n"
        "【답변 금지】\n"
        "그 외 모든 질문. 일반 상식, 시사, 날씨, 코딩, 번역, 창작, 추천(음식·여행·제품),\n"
        "사내 자료와 무관한 조언은 답하지 않는다. 부분적으로도 답하지 않는다.\n"
        "이 경우 정확히 다음 문장만 출력한다:\n"
        "\"사내 자료와 관련된 질문에만 답변할 수 있습니다. 사내 문서 검색이나 파일 분석을 요청해 주세요.\"\n\n"
        "판단이 애매하면 답변 금지 쪽을 택한다."
    )
    try:
