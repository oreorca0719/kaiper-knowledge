...
# 【최적값은 코퍼스 크기에 따라 이동한다 — 실측】
#     142 chunk (문서 3개)  -> k=10
#     484 chunk (문서 17개) -> k=3
# chunk 가 많아질수록 한쪽 랭킹에만 등장하는 후보가 늘어난다. 큰 k 가 주는
# "양쪽 합의" 보너스가 "한쪽에서 강한" 정답을 점점 더 밀어내므로 k 를 낮춰야
# 한다. 코퍼스를 크게 늘리면 재측정이 필요한 상수다.
#
# 484 chunk / 310문항 실측 (McNemar 짝 검정):
#     k=10 -> k=3   r@5  86.8% -> 89.7%  불일치 2:11  p=0.022  유의
#                   r@20 97.7% -> 97.4%  불일치 1:0   (1문항 손실)
#                   gap r@5 81.2% -> 85.9%  불일치 1:7  p=0.070
#     k=2, k=1 은 더 낫지도 않고 유의하지도 않다 (p=0.057, 0.118).
_RRF_K = int(os.getenv("RRF_K", "3"))
